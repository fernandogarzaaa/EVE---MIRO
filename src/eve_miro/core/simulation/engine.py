...
class AgentAction(BaseModel):
    ...
    t: datetime
    hour: int
    action: Literal["stay", "evacuate", "shelter", "stuck"]
    wind_speed: float
    congestion: float
    warning_active: bool
...
class StubSimulationEngine:
    ...
    async def initialize(self, world: WorldState, population: Population) -> Simulation:
        ...
        personas = generate_population(n, rng)
    ...
    async def step(self, simulation: Simulation) -> SimulationStep:
        ...
        for p in simulation.personas:
            ...
            if want:
                ...
                if congestion > 0.45 and (self_ev is False) and (delay_bad or not p.vehicle_access):
                    act: str = "stuck"
                    stuck += 1
